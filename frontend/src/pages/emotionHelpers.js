...
const VALID_EMOTIONS = [
  ...
  "happy", "sad", "neutral/calm", "neutral",
];
...
export function formatTopEmotion(emotion) {
  ...
  try {
      ...
      const mapped = emotion
        .map((e) => {
          ...
          if (typeof e === "object") {
            ...
            if (lbl) {
              return [normalizeLabel(lbl), normalizeScore(sc) ?? 0];
            }
          }

          if (typeof e === "string") {
            return [normalizeLabel(e), 0];
          }
    ...
    if (typeof emotion === "object") {
      if ("label" in emotion && "score" in emotion) {
        return {
          label: normalizeLabel(emotion.label),
          score: normalizeScore(emotion.score) ?? 0,
        };
      }

      if ("probs" in emotion && typeof emotion.probs === "object") {
        const entries = Object.entries(emotion.probs).filter(
          ([k, v]) =>
            VALID_EMOTIONS.includes(normalizeLabel(k)) &&
...
export function getTopEmotionLabel(emotion) {
  ...
  if (!cleanEmotion) return null;

  const top = formatTopEmotion(cleanEmotion);
  if (!top) return null;
...
export function renderEmojiLabelForEmotion(emotion) {
  ...
  if (!VALID_EMOTIONS.includes(label)) return null;
