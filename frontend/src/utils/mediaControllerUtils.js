...
export function syncPreview({ localVideoEl, localStream, placeholderStream, localMirrorEnabled }) {
...
export function createPlaceholderVideoTrack() {
    try {
        ...
        console.warn("[mediaControllerUtils] createPlaceholderVideoTrack failed:", err);
        return null;
    }
}
...
export function stopAndCleanupPlaceholder(track) {
    ...
    try {
        if (track.__placeholderStream) {
            track.__placeholderStream.getTracks().forEach(t => { try { t.stop(); } catch { } });
            track.__placeholderStream = null;
        }
    } catch { }
}

export function attachTrackEndHandler(track, kind, { socketRef, localStream, localVideoEl, localMirrorEnabled }) {
    if (!track) return;
    const handler = () => {
        try {
            const present = localStream?.getTracks().some(t => t.id === track.id);
            if (!present) return;
        } catch { }
        if (kind === "audio") {
...
export async function replaceTrackInPeers(track, kind, { pcsRef, localStream }) {
    ...
    for (const pc of pcs) {
        try {
            ...
            console.warn("[mediaControllerUtils] replaceTrackInPeers error for pc:", err);
        }
    }
}
...
export function replaceLocalTrack(newTrack, kind, { localStream, localVideoEl, localMirrorEnabled, socketRef }) {
    if (!localStream) {
    ...
    try {
        ...
        console.warn("[mediaControllerUtils] replaceLocalTrack: removing old tracks failed:", err);
    }
    try {
        localStream.addTrack(newTrack);
        attachTrackEndHandler(newTrack, kind, { socketRef, localStream, localVideoEl, localMirrorEnabled });
    } catch (err) {
        console.warn("[mediaControllerUtils] replaceLocalTrack: addTrack failed:", err);
        try { newTrack.stop(); } catch { }
        return;
    }
    if (kind === "video") {
...
export function stopAndRemoveTracks(kind, { localStream, pcsRef, localVideoEl, localMirrorEnabled }) {
    ...
    try {
        ...
        console.warn("[mediaControllerUtils] stopAndRemoveTracks: local track removal failed:", err);
    }
    const pcs = Object.values(pcsRef ?? {});
    for (const pc of pcs) {
        try {
            ...
            const matched = txs.filter(t => {
                try {
                    return t.sender?.track?.kind === kind || t.receiver?.track?.kind === kind || t.kind === kind;
                } catch { return false; }
            });
            if (matched.length > 0) {
                matched.forEach(t => {
            ...
            console.warn("[mediaControllerUtils] stopAndRemoveTracks pc loop error:", err);
        }
    }
...
export function refreshSafariPreview({ localVideoEl, localStream, placeholderStream, localMirrorEnabled }) {
...
export function runExternalCleaners(kind, { externalCleaners, localStream }) {
    try {
        const rr = externalCleaners.recordersRef?.current;
    ...
    try {
        if (typeof externalCleaners.removeAnalyzerFn === "function") {
            try { externalCleaners.removeAnalyzerFn(kind); } catch { }
        }
    } catch { }
    try {
