...
export async function handleEmotionChunk(socket, payload = {}) {
    const start = startTimer();
    try {
        ...
        const chunkRateKey = KEYS.chunkRate(participantId);
        if (await isSocketRateLimited(participantId, chunkRateKey, SOCKET_CHUNK_RATE_MAX, SOCKET_CHUNK_RATE_WIN_SEC)) {
            return { ok: false, reason: "rate_limited" };
        }

        if (payload.type && !ALLOWED_CHUNK_TYPES.includes(payload.type)) {
            return { ok: false, reason: "invalid_type" };
        }

        const key = `${String(meetingId).trim().toUpperCase()}__${String(participantId)}`;
        let meta = await getPartialMeta(key);

        if (!meta) {
            if (totalChunks <= 0 || totalChunks > MAX_CHUNKS) {
                return { ok: false, reason: "invalid_totalChunks" };
            }
            const dir = makeUploadDir(key);
            await mkdirp(dir);
            meta = { dir, totalChunks, receivedBytes: 0, receivedCount: 0, filename, createdAt: Date.now(), maxBytes };
            await setPartialMeta(key, meta);
        }
        ...
        if (bufChunk.length > PARTIAL_UPLOAD_MAX_BYTES) {
            return { ok: false, reason: "chunk_too_large" };
        }

        if (meta.receivedBytes + bufChunk.length > meta.maxBytes) {
            await deletePartialMeta(key);
            await unlink(meta.dir);
            return { ok: false, reason: "too_large" };
        }

        if (seq < (cfg.upload?.chunkSeqMin ?? 0) || seq >= meta.totalChunks) {
        ...
        return { ok: true, seq, receivedCount: meta.receivedCount };
    } finally {
        endTimer(LATENCY_LABELS.EMOTION_CHUNK, start, {
            meetingId,
            participantId,
            seq,
        });
...
export async function handleEmotionChunkAbort(socket, metaReq = {}) {
    ...
    if (!meetingId || !participantId) return { ok: false, reason: "missing" };

    const key = `${String(meetingId).trim().toUpperCase()}__${String(participantId)}`;
    const meta = await getPartialMeta(key);
...
export async function handleEmotionChunkComplete(socket, io, metaReq = {}, sendToEmotionService) {
    const start = startTimer();
    try {
        const meetingId = metaReq.meetingId || metaReq.meeting_id || metaReq.meeting || socket.data?.meetingCode;
        const participantId = metaReq.participantId || metaReq.participant_id || metaReq.from || socket.data?.userId;
        ...
        if (!meetingId || !participantId) return { ok: false, reason: "missing" };
        if (!ALLOWED_CHUNK_TYPES.includes(type)) return { ok: false, reason: "invalid_type" };

        const mid = String(meetingId).trim().toUpperCase();
        const key = `${mid}__${String(participantId)}`;
        const meta = await getPartialMeta(key);

        if (!meta) return { ok: false, reason: "missing_upload" };
        ...
        if (missing.length > 0) return { ok: false, reason: "missing_chunks", missing };
        ...
        await new Promise((res, rej) => { writeStream.end(res); writeStream.on("error", rej); });
        ...
        if (finalStat.size > meta.maxBytes) {
            await deletePartialMeta(key);
            await unlink(meta.dir);
            return { ok: false, reason: "too_large_final" };
        }
        ...
        let emotionRes;
        try {
            emotionRes = await sendToEmotionService(mid, String(participantId), finalPath, type, {
                mime: metaReq.mime,
                filename: finalFilename,
                timeoutMs: metaReq.timeoutMs || undefined,
            });

            logEvent(LATENCY_LABELS.EMOTION_RESULT, {
                meetingId: mid,
                participantId: String(participantId),
                type,
                emotion: emotionRes?.emotion || "unknown",
                confidence: emotionRes?.confidence || null,
            });

        } catch (svcErr) {
            await unlink(meta.dir);
            return { ok: false, reason: "service_error", message: svcErr?.message || String(svcErr) };
        }

        try {
            await updateMeetingAnalytics(mid, { emotionScores: emotionRes });
        } catch (dbErr) {
            log.warn("analytics persist failed", { err: dbErr?.message });
        }

        const stateArr = await getState(mid);
        const hostSocketId = stateArr?.[0] ?? null;
...
export async function handleEmotionUpdate(socket, io, data) {
    const code = socket.data?.meetingCode;
    if (!code || typeof data !== "object") return;

    await updateMeetingAnalytics(code, { emotionScores: data });

    const stateArr = await getState(code);
    const hostSocketId = stateArr?.[0] ?? null;
